...
const packages = [
  ...
  { credits: 24, price: 15900, per: '₩663', label: '가장 인기', featured: true },
  ...
  const handlePayment = () => {
    ...
    IMP.request_pay({
      ...
      if (rsp.success) {
        ...
        if (data.success) {
          setCredits(c => c + data.credits)
          setShowPricing(false)
          alert(`✅ ${data.credits}회 크레딧이 충전됐습니다!`)
        }
      } else {
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1rem', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ background: '#EEEDFE', borderRadius: 20, padding: '6px 14px', fontSize: 13, color: '#534AB7', fontWeight: 500 }}>
            크레딧 {credits}회
          </div>
      ...
      {showPricing && (
        <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: 12, padding: '1.25rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: '1rem' }}>
            {packages.map(pkg => (
              <div key={pkg.credits} onClick={() => setSelectedPkg(pkg.credits)}
                ...
                <div style={{ fontSize: 24, fontWeight: 600, color: '#1e1e2e' }}>{pkg.credits}<span style={{ fontSize: 13, color: '#888' }}>회</span></div>
          ...
          <button onClick={handlePayment} disabled={payLoading}
            ...
            {payLoading ? '결제 처리 중...' : `💳 ${selectedPkg}회 크레딧 결제하기 (₩${packages.find(p => p.credits === selectedPkg)!.price.toLocaleString()})`}
          </button>
        </div>
      )}
            ...
            {grades.map(g => (
              <button key={g} onClick={() => setGrade(g)}
                style={{ padding: '6px 14px', borderRadius: 20, border: '1px solid', fontSize: 13, cursor: 'pointer',
                  background: grade === g ? '#534AB7' : '#fff', color: grade === g ? '#fff' : '#666',
                  borderColor: grade === g ? '#534AB7' : '#ddd' }}>{g}</button>
            ))}
          </div>
        </div>
        <div style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {types.map(t => (
              <button key={t} onClick={() => setType(t)}
                style={{ padding: '6px 14px', borderRadius: 20, border: '1px solid', fontSize: 13, cursor: 'pointer',
                  background: type === t ? '#534AB7' : '#fff', color: type === t ? '#fff' : '#666',
                  borderColor: type === t ? '#534AB7' : '#ddd' }}>{t}</button>
            ))}
          </div>
        </div>
